...
public class AudioplayersHandler implements MethodCallHandler {

    private static final Logger LOGGER = Logger.getLogger(AudioplayersHandler.class.getCanonicalName());
    ...
    AudioplayersHandler(final MethodChannel channel, Context context) {
    ...
    private static final class UpdateCallback implements Runnable {
        ...
        private final WeakReference<AudioplayersHandler> audioplayersPlugin;

        private UpdateCallback(final Map<String, Player> mediaPlayers,
                               final MethodChannel channel,
                               final Handler handler,
                               final AudioplayersHandler audioplayersHandler) {
            this.mediaPlayers = new WeakReference<>(mediaPlayers);
            this.channel = new WeakReference<>(channel);
            this.handler = new WeakReference<>(handler);
            this.audioplayersPlugin = new WeakReference<>(audioplayersHandler);
        }
        ...
        public void run() {
            ...
            final AudioplayersHandler audioplayersHandler = this.audioplayersPlugin.get();

            if (mediaPlayers == null || channel == null || handler == null || audioplayersHandler == null) {
                if (audioplayersHandler != null) {
                    audioplayersHandler.stopPositionUpdates();
                }
                return;
            }
